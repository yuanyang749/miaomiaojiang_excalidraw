---
name: miaomiaojiang_excalidraw
description: Create polished Excalidraw diagrams that argue visually, not just display boxes. Use this whenever the user asks to 画图、生成架构图、流程图、时序图、对比图、脑图、系统图，uses slash-style prompts like /excalidraw, /diagram, /draw, or casually mentions a business/technical scenario and wants “用合适的图表帮我讲清楚/画出来/梳理一下”. In OpenClaw chats, prefer proactive diagram selection: infer the most suitable diagram type from the scenario even when the user does not name a specific chart type.
---
...
The palette file currently defines multiple themes. Choose one theme per diagram and apply it consistently.

Use these defaults:
- **Default / Universal** — neutral explanatory diagrams, the safe default
- **技术指南针 / Tech Compass** — technical articles, architecture, tutorials, comparison diagrams
- **小红书 IP / Warm Creator** — warm, social, lifestyle, story-driven or creator-facing content

If the user explicitly mentions a brand or channel, prefer the matching theme. Otherwise default to **Default / Universal**.

### Style phrase mapping

If the user asks for a named style, interpret it like this unless they override it:
- **白板风** → slightly hand-drawn (`roughness: 1`) with visible texture
- **讲义风** → light background color + soft visible stripe texture + structured panels
- **草稿风** → stronger texture and more hand-drawn looseness
- **教材风** → Chinese-first, more formal hierarchy, teaching-oriented density

---

## OpenClaw Shortcut Mode

In OpenClaw chats, the user may not give a full spec. They may simply say things like:
- “用合适的图表帮我解释一下这个业务”
- “帮我画一下这个系统怎么跑的”
- “把这个方案梳理成一张图”
- “给我一张适合发文章的图”

When that happens, **do not block on a long questionnaire by default**. Infer the right diagram type from the scenario, fill in reasonable defaults, and move forward unless the request is ambiguous in a way that would materially change the result.

### Default behavior for underspecified requests

If the user gives only a business or technical scenario, infer these silently:
- **Diagram type** from the dominant intent
- **Audience** from context (default: educated internal audience / technical readers)
- **Depth** from risk and purpose (default: simple for quick explanation, comprehensive for architecture/protocol/tutorial content)
- **Style** as clean, modern, explanatory, suitable for docs/articles/Telegram sharing
- **Icon strategy** as lightweight symbolic shapes unless the user explicitly asks for official brand logos

Only ask follow-up questions if one of these is genuinely unclear:
- The scenario could map to multiple very different diagram types
- A factual/technical diagram requires specs you do not yet know
- The deliverable format matters (e.g. comparison vs sequence vs architecture) and changes the whole composition

### Scenario → recommended diagram type

Use these defaults when the user does not specify a chart type:

| Scenario | Preferred diagram |
|----------|-------------------|
| Explaining a user/business process, approvals, order flow, onboarding, payment path | **Business flow / process diagram** |
| Explaining services, modules, data stores, third-party integrations, AI agent components | **System architecture diagram** |
| Explaining message passing, callbacks, request lifecycle, event order, handoffs between actors | **Sequence / timeline diagram** |
| Comparing two products, approaches, architectures, memory systems, vendor choices | **Comparison diagram** |
| Explaining a concept, capability map, planning structure, decomposition tree | **Concept map / mind map** |
| Summarizing a topic for an article, slide, or tutorial as one strong visual | **One-page explanatory diagram** |

### Common business scenes to support well

Be especially ready to visualize these without requiring the user to over-specify them:
- User journey / conversion funnel
- Order / payment / fulfillment / notification flow
- Approval workflows / CRM handoff / lead management
- Course delivery / education operations / teaching system flow
- AI agent workflows / tool calling / memory + execution layering
- Backend architecture / event-driven systems / queue-based processing
- Product option comparison / migration decisions / system positioning
- Article illustrations that need a single clear “explainer” visual

### Output mindset for shortcut mode

When the request is brief, optimize for **first useful draft**, not exhaustive elicitation.
A good default draft should:
- choose the right diagram family,
- surface the likely main actors and relationships,
- make one core point visually obvious,
- and be easy to refine in one follow-up turn.
...
```bash
cd ~/.openclaw/workspace/skills/miaomiaojiang_excalidraw/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd ~/.openclaw/workspace/skills/miaomiaojiang_excalidraw/references
uv sync
